#!/bin/bash
#SBATCH --job-name=188bwa_vcf              # create a short name for your job
...
in_gatk=/Genomics/grid/users/yushit/.local/bin/gatk-new/gatk-4.1.3.0
...
bwa mem -t 15 /Genomics/ayroleslab2/alea/ref_genomes/hg38/hg38_all_chr.fa $out_fastq1 $out_fastq2 > $out_sam
PATH=$originalpath
...
samtools sort -m 36G -o $out_bam_sorted $out_bam
...
$in_sambamba index -t 15 $out_bam_readgroups
...
$in_sambamba index -t 15 $out_bam_recalibrat

# Step III

echo 'calling GATK variants as VCF and zip...'

$in_gatk/gatk --java-options "-Xmx60g" HaplotypeCaller -R $in_genome -I $out_bam_recalibrat -O $out_g_vcf -ERC GVCF --max-alternate-alleles 2 --native-pair-hmm-threads 15
